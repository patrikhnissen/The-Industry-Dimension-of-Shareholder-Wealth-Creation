...
def table6_bessembinder_style(stock_df, df_monthly):
    ...
    summary.to_csv("table_industry_wc_bessembinder_style_90_24.csv", index=False)
    print("Saved: table_industry_wc_bessembinder_style_90_24.csv")
...
def table7_within_industry_concentration(stock_df):
    ...
    result.to_csv("table_within_industry_concentration_90_24.csv", index=False)
    print("Saved: table_within_industry_concentration_90_24.csv")
...
def table8_return_distribution(stock_df):
    ...
    result.to_csv("table_return_distribution_90_24.csv", index=False)
...
def figure2_return_distributions(stock_df):
    ...
    ax.axvline(x=med,  color=RED,  linewidth=1.2, linestyle='--',
               label=f'Median: {med:.0%}')
    ax.axvline(x=mean, color=GREY, linewidth=1.0, linestyle=':',
               label=f'Mean: {mean:.0%}')
    ...
    ax.legend(fontsize=9, framealpha=0.4, loc='upper right',
              title=f'N={len(bhr):,}  ({n_clipped} clipped)',
              title_fontsize=8)
    ...
    plt.tight_layout()
    fig.savefig("figure_return_distributions.png", dpi=200, bbox_inches='tight')
    plt.close(fig)
    print("Saved: figure_return_distributions.png")


# FIGURE: CUMULATIVE NET WEALTH CREATION FOR BUSINESS EQUIPMENT AND UTILITIES
def figure5_lottery_vs_yield(stock_df):
    plt.rcParams.update({
        'font.family':       'serif',
        'axes.spines.top':   False,
        'axes.spines.right': False,
    ...
    for ax, ind in zip(axes, ['Business Equipment', 'Utilities']):
        sub        = stock_df[stock_df['FF12'] == ind].copy()
        sub_sorted = sub.sort_values('wealth_creation', ascending=False)
        net_wc     = sub['wealth_creation'].sum()

        cum_wc_pct = sub_sorted['wealth_creation'].cumsum() / net_wc * 100
        n_firms    = np.arange(1, len(sub_sorted) + 1)

        x_vals = np.concatenate([[0], n_firms])
        y_vals = np.concatenate([[0], cum_wc_pct.values])

        ax.plot(x_vals, y_vals, color=BLUE, linewidth=2.2)
        ax.axhline(y=100, color=DARKGREY, linewidth=0.8,
                   linestyle='--', alpha=0.5)

        ax.set_xlabel('Number of firms', fontsize=10, color=DARKGREY)
        ax.set_ylabel('Cumulative share of net wealth creation (%)',
                      fontsize=10, color=DARKGREY)
        ax.yaxis.set_major_formatter(
            mticker.PercentFormatter(xmax=100, decimals=0))
        ax.tick_params(axis='both', labelsize=9, colors=DARKGREY)
        ax.set_title(ind, fontsize=11, color=DARKGREY, pad=10)

    fig.suptitle(
        'Within-Industry Wealth Creation Concentration\n'
        'Firms ranked from highest to lowest wealth creation. 1990-2024.',
        fontsize=10, color=DARKGREY, y=1.02
    )

    plt.tight_layout()
    fig.savefig("figure_lottery_vs_yield.png", dpi=200, bbox_inches='tight')
    plt.close(fig)
    print("Saved: figure_lottery_vs_yield.png")

# FIGURE: CUMULATIVE NET WEALTH CREATION FOR ALL INDUSTRIES
def figure_appendix_all_industries(stock_df):
    plt.rcParams.update({
        'font.family':       'serif',
        'axes.spines.top':   False,
        'axes.spines.right': False,
    ...
    for i, ind in enumerate(FF12_ORDER):
        ax  = axes_flat[i]
        sub = stock_df[stock_df['FF12'] == ind].copy()

        if len(sub) == 0:
            ax.set_title(ind, fontsize=9, color=DARKGREY, pad=6)
            ax.set_yticks([])
            continue
        ...
        ax.plot(x_vals, y_vals, color=BLUE, linewidth=1.8)
        ax.axhline(y=100, color=DARKGREY, linewidth=0.8,
                   linestyle='--', alpha=0.5)

        ax.set_xlabel('Number of firms', fontsize=8, color=DARKGREY)
        ax.set_ylabel('Cum. share of net WC (%)', fontsize=8, color=DARKGREY)
        ax.yaxis.set_major_formatter(
            mticker.PercentFormatter(xmax=100, decimals=0))
        ax.tick_params(axis='both', labelsize=8, colors=DARKGREY)
        ax.set_title(f'{ind}  (N={len(sub):,})',
                     fontsize=9, color=DARKGREY, pad=6)

    fig.suptitle(
        'Appendix Figure: Within-Industry Cumulative Net Wealth Creation\n'
        'All 12 FF industries. Firms ranked from highest to lowest wealth creation. 1990-2024.',
        fontsize=11, color=DARKGREY, y=1.01
    )

    plt.tight_layout()
    fig.savefig("figure_appendix_all_industries_wc.png",
                dpi=200, bbox_inches='tight')
    plt.close(fig)
    print("Saved: figure_appendix_all_industries_wc.png")
...
if __name__ == '__main__':
    ...
    figure5_lottery_vs_yield(stock_df)
    figure_appendix_all_industries(stock_df)

    print("\nDone. Outputs:")
    print("  table6_industry_wc_bessembinder_style_90_24.csv")
    print("  table7_within_industry_concentration_90_24.csv")
    print("  table8_return_distribution_90_24.csv")
    print("  figure2_return_distributions.png")
    print("  figure5_lottery_vs_yield.png")