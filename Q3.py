...
def build_monthly_panel(df):
    ...
    df['ME_LAG'] = df.groupby('PERMNO')['MCAP'].shift(1)
    df.loc[df['ME_LAG'].isna(), 'ME_LAG'] = df.loc[df['ME_LAG'].isna(), 'MCAP']
    ...
    for m in months:
        sub          = df[df['date'] == m]
        rets         = sub['RET'].values.astype(np.float64)
        me_lag       = sub['ME_LAG'].values.astype(np.float64)
        industries   = sub['FF12'].values
        panel[m] = {
            'rets':       rets,
            'me_lag':     me_lag,
        ...
        for j, ind in enumerate(industries):
            ind_map.setdefault(ind, []).append(j)
        ind_idx[m] = {k: np.array(v, dtype=np.int64) for k, v in ind_map.items()}

    return months, panel, ind_idx


# UNCONSTRAINED BOOTSTRAP
def bootstrap_horizon(months, panel, market_monthly_arr, tbill_monthly_arr,
                      portfolio_sizes, n_sims, rng, horizon_months,
                      full_sample=False):
    ...
    for n in portfolio_sizes:
        ...
        for i in range(n_sims):
            if full_sample:
                start_idx = 0
                end_idx   = n_months_total
            else:
                if n_months_total - h <= 0:
                    start_idx = 0
                else:
                    start_idx = int(rng.integers(0, n_months_total - h + 1))
                end_idx = start_idx + h
            ...
            for t in range(start_idx, end_idx):
                ...
                n_avail = p['n']
                if n_avail == 0:
                    log_mkt   += np.log1p(market_monthly_arr[t])
                    log_tbill += np.log1p(tbill_monthly_arr[t])
                    continue
                size_draw = min(n, n_avail)
                idx_draw  = rng.choice(n_avail, size=size_draw, replace=False)
                w         = p['me_lag'][idx_draw]
                w_sum     = w.sum()
                if w_sum > 0:
                    w = w / w_sum
                else:
                    w = np.ones(size_draw) / size_draw
                r_port = float(np.dot(w, p['rets'][idx_draw]))

                log_port  += np.log1p(r_port)
                log_mkt   += np.log1p(market_monthly_arr[t])
                log_tbill += np.log1p(tbill_monthly_arr[t])

            port_rets[i]   = np.exp(log_port)  - 1
            market_rets[i] = np.exp(log_mkt)   - 1
            tbill_rets[i]  = np.exp(log_tbill) - 1
        ...
        print(f"  n={n:>4}  >Tbill: {pct_tbill:.1%}  >Mkt: {pct_market:.1%}")

    return pd.DataFrame(results)


# INDUSTRY-STRATIFIED BOOTSTRAP
FF12_INDUSTRIES = [
...
def bootstrap_stratified_fullsample(months, panel, ind_idx,
                                    market_monthly_arr, tbill_monthly_arr,
                                    n_sims, rng):

    n_months_total = len(months)

    # Determine which industries are populated in the panel
    industries_present = set()
    for m in months:
        industries_present.update(ind_idx[m].keys())
    available = [i for i in FF12_INDUSTRIES if i in industries_present]
    ...
    for k in range(1, n_avail + 1):
        ...
        for i in range(n_sims):
            chosen_inds_idx = rng.choice(n_avail, size=k, replace=False)
            chosen_inds     = [available[j] for j in chosen_inds_idx]

            log_port  = 0.0
            log_mkt   = 0.0
            log_tbill = 0.0

            for t in range(n_months_total):
                m       = months[t]
                p       = panel[m]
                ind_map = ind_idx[m]
                ...
                for ind in chosen_inds:
                    pool = ind_map.get(ind)
                    if pool is None or len(pool) == 0:
                        continue
                    pos = pool[rng.integers(0, len(pool))]
                    stock_rets.append(p['rets'][pos])
                    stock_w.append(p['me_lag'][pos])
                ...
                stock_rets = np.array(stock_rets, dtype=np.float64)
                stock_w    = np.array(stock_w,    dtype=np.float64)
                w_sum      = stock_w.sum()
                if w_sum > 0:
                    w = stock_w / w_sum
                else:
                    w = np.ones(len(stock_rets)) / len(stock_rets)

                r_port = float(np.dot(w, stock_rets))
        ...
        print(f"  K={k:>2}  >Tbill: {pct_tbill:.1%}  >Mkt: {pct_market:.1%}")
...
def print_table11(results_by_horizon):
    ...
    print("\n" + "="*115)
    print("BOOTSTRAP RESULTS — VALUE-WEIGHTED RANDOM PORTFOLIOS (monthly resampling)")
...
def print_table12(strat_df):
    print("\n" + "="*72)
    print("INDUSTRY-STRATIFIED BOOTSTRAP (monthly resampling, full sample)")
    print("K = 1 to 12 (Other inkluderet). Value-weighted within month.")
    print("="*72)
...
def plot_diversification_curves(results_by_horizon, strat_df):
    ...
    panel_configs = [
        ('% > T-bill',    'Probability of beating T-bills (%)',
         ...
         'Sample 1990-2024. Monthly resampled VW portfolios.',
...
if __name__ == '__main__':
    ...
    months, panel, ind_idx = build_monthly_panel(df)
    print(f"  Months in panel: {len(months)}")
    print(f"  Total stocks:    {df['PERMNO'].nunique():,}")

    # Align FF benchmark arrays with panel months
    ff_indexed         = ff_market.set_index('date_ff')
    market_monthly_arr = np.array(
        [float(ff_indexed.loc[m, 'Mkt']) if m in ff_indexed.index else 0.0
         for m in months], dtype=np.float64)
    tbill_monthly_arr  = np.array(
        [float(ff_indexed.loc[m, 'RF']) if m in ff_indexed.index else 0.0
         for m in months], dtype=np.float64)
    ...
    for label, (h_months, full) in HORIZONS.items():
        ...
        res = bootstrap_horizon(
            months, panel, market_monthly_arr, tbill_monthly_arr,
            PORTFOLIO_SIZES, N_SIMS, rng, h_months,
            full_sample=full
        )
